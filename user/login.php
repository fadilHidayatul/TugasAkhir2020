        ...
        try{
            ...
            if ($statement->rowCount()) {
                ...
                if($chk_pass){
                    ...
                    $get_nis = "SELECT * FROM tb_nis WHERE nis = :nis";
                    $getNIS = $conn->prepare($get_nis);
                    $getNIS->bindParam(":nis", $nis);
                    $getNIS->execute();
                    $row_nis = $getNIS->fetch(PDO::FETCH_ASSOC);

                    $id_nis = $row_nis['id_nis'];
                    $kelas = $row_nis['kelas'];

                    $query = "UPDATE tb_user SET token=:token , id_nis=:id_nis , kelas=:kelas
                              WHERE nis=:nis";
                    $up_token = $conn->prepare($query);
                    $up_token->bindParam(":token",$token);
                    $up_token->bindParam(":id_nis",$id_nis);
                    $up_token->bindParam(":kelas",$kelas);
